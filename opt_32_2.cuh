// Swap shared memory transform kernels to register-based transform kernels
template <int kernel, int block_y>
__launch_bounds__(32 * block_y, 1)
__global__ void opt2wl79_32x32x32(float *in) {
        ...
        for (int batch_z = 0; batch_z < num_batches_z; ++batch_z) {
              ...
              for (int z = 0; z < planes / block_y; ++z) {
                      if (kernel == 0) { 
                        ds79_compute<32>(
                            &smem[snx * idx + snxy * (idy + z * block_y)], 1);
                      } else {
                      us79_compute<32>(
                          &smem[snx * idx + snxy * (idy + z * block_y)], 1);
                        }
              ...
              for (int z = 0; z < planes / block_y; ++z) {
                      if (kernel == 0) {
                              ds79_compute<32>(
                                  &smem[idx + snxy * (idy + z * block_y)], snx);
                      } else {
                              us79_compute<32>(
                                  &smem[idx + snxy * (idy + z * block_y)], snx);
                      }
       ...
       const int num_batches_y = 32 / planes;
        ...
        for (int batch_y = 0; batch_y < num_batches_y; ++batch_y) { 
              ...
              for (int y = 0; y < planes / block_y; ++y) {
                      if (kernel == 0) {
                        ds79_compute<32>(
                            &smem[idx + snxy * (idy + y * block_y)], snx);
                      } else {
                        us79_compute<32>(
                            &smem[idx + snxy * (idy + y * block_y)], snx);
                      }
...
template <int mode>
void opt2wl79_32x32x32_h(float *in, const int bx, const int by, const int bz) {
        const int block_y = 4;
        dim3 threads(32, block_y, 1);
        dim3 blocks(bx, by, bz);
        opt2wl79_32x32x32<mode, block_y><<<blocks, threads>>>(in);
